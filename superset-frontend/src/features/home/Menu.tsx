...
export function Menu({
  ...
  enum Paths {
    ...
    Collections = '/superset/collections',
  }
  ...
  useEffect(() => {
    ...
    switch (true) {
      case path.startsWith(Paths.Collections):
        setActiveTabs(['Collections']);
        break;
...
export default function MenuWrapper({ data, ...rest }: MenuProps) {
  ...
  // Ensure Collections appears as the first menu item
  const collectionsIndex = newMenuData.menu.findIndex(
    (item: MenuObjectProps) => item.label === 'Collections',
  );
  
  let collectionsItem: MenuObjectProps;
  if (collectionsIndex >= 0) {
    // Collections exists, remove it from current position
    collectionsItem = newMenuData.menu.splice(collectionsIndex, 1)[0];
  } else {
    // Collections doesn't exist, create it
    collectionsItem = {
      label: 'Collections',
      url: '/superset/collections/',
      icon: undefined,
    };
  }
  
  // Insert Collections at the beginning (first position)
  newMenuData.menu.unshift(collectionsItem);
