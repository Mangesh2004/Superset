...
class SupersetAppInitializer:  # pylint: disable=too-many-public-methods
    ...
    def init_views(self) -> None:
        ...
        # Add Collections menu link as a top-level nav item (next to Dashboards, Charts, Datasets)
        # Note: no category provided -> becomes a root/top-level menu entry
        appbuilder.add_link(
            "Collections",
            label=_("Collections"),
            href="/superset/collections/",
            icon="fa-folder-open",
            category="",
            category_icon="",
        )
    ...
    def _init_database_dependent_features(self) -> None:
        ...
        # Initialize Collections feature if enabled
        try:
            from superset.collections import register_collections_api
            register_collections_api(self.superset_app)
        except ImportError:
            logger.debug("Collections module not available")
        except Exception as ex:
            logger.error(f"Failed to initialize Collections: {ex}")
