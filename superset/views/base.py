...
def cached_common_bootstrap_data(  # pylint: disable=unused-argument
    ...
    # Add hybrid auth configuration
    if hasattr(appbuilder.sm, 'get_hybrid_auth_config'):
        hybrid_config = appbuilder.sm.get_hybrid_auth_config()
        frontend_config["HYBRID_AUTH_CONFIG"] = hybrid_config
        frontend_config["AUTH_TYPE"] = "HYBRID"  # Override for hybrid auth
    else:
